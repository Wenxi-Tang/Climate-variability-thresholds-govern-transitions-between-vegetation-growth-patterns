...
#### File: combined_list-VPD+CO2+SM.Rdata

**Description:** The primary input time-series data file required for `01_Parallel_SlidingWindow_VAR_GPP.R`. It contains detrended and standardized vegetation indices and climate variables for global grid cells.

#### File: VARmodel_Result_1982_2020.zip

**Description:** The intermediate output generated by `01_Parallel_SlidingWindow_VAR_GPP.R`. It contains the raw Impulse Response Functions (IRF) and model stability statistics derived from the VAR modeling. This file serves as the input for the pattern classification in `02_Parallel_Response_Pattern_Classification.R`.

#### File: VegtationResponseType_1982_2020.zip

**Description:** The output directory generated by `02_Parallel_Response_Pattern_Classification.R`. It contains GeoTIFF raster files representing the spatial distribution of the classified vegetation response types (VGC, VSO, Stable).

#### Folder: Data

**Description:** This directory contains additional auxiliary data required for the analysis.

* **LSP_month**: Land Surface Phenology data indicating the Start of Season (SOS) and End of Season (EOS).
* **SHAP_data**: The aggregated dataset used for the Python Random Forest model.
