...
get_available_cores <- function(max_cores = 80) {
  ...
  available_cores <- min(max_cores, max(30, floor(total_cores - load_avg)))
  ...
  tryCatch({
    ...
    for (g_i in seq_along(lab_group)) {
      ...
      tryCatch({
        ...
        season_GS <- as.numeric(df.GS_lab$Num_GS_Months)
        ...
        if(anyNA(data.test$Year_lab)){
          ...
          for (season_year_i in 1:season_year) {
            ...
            season_year_b <- season_year_a + (season_GS -1)
        ...
        if(any(is.na(data))){
          data <- complete(mice(data, m = 1, method = 'pmm'))
        ...
        for (variable_i in 1:ncol(data)) {
          for(typename in c("none","trend","drift")){
            ...
            ADF_all <- dplyr::bind_rows(ADF_all, adf_results)
        ...
        var_model <- VAR(data, p = 1, type = "both", season = season_GS)
        ...
        johansen_all <- dplyr::bind_rows(johansen_all, johansen_result_df)
        ...
        # Store
        var_sel <- VARselect(data, type="both", season=season_GS, lag.max=season_GS)
        lag_RS <- data.frame(var_sel$criteria)
        lag_RS$best_AIC_lagnum <- var_sel$selection["AIC(n)"]
        lag_RS$lag_num <- lag_num
        ...
        lag_RS_all <- dplyr::bind_rows(lag_RS_all, lag_RS)
        ...
        var_R2.all <- dplyr::bind_rows(var_R2.all, var_R2)
        ...
        var_root_all <- dplyr::bind_rows(var_root_all, var_root_df)
        ...
        irf_all <- dplyr::bind_rows(irf_all, irf_est_low_up)
        ...
        if(VI_name == "GPP" && exists("irf_est_low_up") && nrow(irf_est_low_up) > 0){
          ...
          if(nrow(gpp_irf) > 0){
            window_gpp_irf <- dplyr::bind_rows(window_gpp_irf, gpp_irf)
        ...
        for (ncol in 1:(ncol(irf_est)-2)) {
          ...
          q_all <- dplyr::bind_rows(q_all, df.q)
          ...
          duration_all <- dplyr::bind_rows(duration_all, time_lag_c)
        ...
        fevd_all <- dplyr::bind_rows(fevd_all, fevd_VI.m)
        ...
        causal_all <- dplyr::bind_rows(causal_all, causal_df)
        ...
        calls <- sys.calls()
        call_stack <- paste(as.character(calls), collapse = " -> ")
        error_msg <- paste0(
          "Error in grid ", i, ": ", e$message, "\n",
          "Traceback (Call Stack): ", call_stack
        )
    ...
    if(VI_name == "GPP" && exists("duration_all") && nrow(duration_all) > 0){
      ...
      if(nrow(gpp_self) > 0){
        window_gpp_response <- dplyr::bind_rows(window_gpp_response, gpp_self)
...
# Create directories + RUN MIAN PROGRESS -----
# History


# VI_name <- "GPP"
# year_list <- c(2005:2011)

# for (yi in 1:length(year_list)) {
# for (clim_name in c("LAI", "NDVI", "SIF")) {

# 10 GPP, 11 NDVI, 12 LAI, 13 SIF
  vi_N <- 10
  VI_name <- "GPP"
  year_starts <- 1982
  year_ends   <- 2020
  window_size <- (year_ends-year_starts) + 1
  
  # year_starts <- year_list[yi]
  # year_ends   <- year_starts + 9
  # window_size <- (year_ends-year_starts) + 1
  # window_size <- 10
  Model <- "History"
  
  for (YEAR_period in seq_along(year_starts)) {
    year_start <- year_starts[YEAR_period]
    year_end <- year_ends[YEAR_period]
    RUN_name <- paste0(Model, '_GroupBy_',group_by_n,'_SlidingWindow_',window_size,'yr_',year_start,"_",year_end)
    ...
    # Main loop ----
    # History
    data_paths <- "./Data/combined_list-VPD+CO2+SM_250419.RData"
    load("./Data/combined_list-VPD+CO2+SM_250419.RData")
    
    for(data_path in data_paths){
      
      if(Model == "History"){
        matched_model <- "Hist"
        cat("\n========== History: ", matched_model, " // ", year_start, "-", year_end, " ==========\n")
      ...
      for(VI_i in c(vi_N)){ # 10 GPP, 11 NDVI, 12 LAI, 13 SIF
        VI_name <- colnames(combined_list[[1]])[vi_N]
        ...
        VI_dir <- file.path(Datedir, paste0(matched_model,"_", VI_name))
          ...
          for(result_idx in seq_along(parallel_results)){
            ...
            if(class(result) != "try-error" && !is.null(result) && result$success){
              ...
              tryCatch({
                ...
                save(a1, 
                     file = file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_VARresult_list.RData")))
                
                a2 <- result$VAR_data_list
                save(a2, 
                     file = file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_VAR_data_list.RData")))
                
                # Save data.frame to .csv file
                write.csv(result$var_R2.all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_var_R2.csv")), row.names = FALSE)
                write.csv(result$lag_RS_all,
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_lag_num_RS.csv")), row.names = FALSE)
                write.csv(result$duration_all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_duration.csv")), row.names = FALSE)
                write.csv(result$causal_all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_causal.csv")), row.names = FALSE)
                write.csv(result$q_all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_q.csv")), row.names = FALSE)
                write.csv(result$ADF_all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_ADF.csv")), row.names = FALSE)
                write.csv(result$johansen_all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_johansen.csv")), row.names = FALSE)
                write.csv(result$var_root_all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_var_root.csv")), row.names = FALSE)
                write.csv(result$irf_all, 
                          file.path(group_dir, paste0(matched_model, "_", VI_name, "_G", group_i, "_window_", window_label, "_irf.csv")), row.names = FALSE)
                ...
                if(nrow(result$window_gpp_response) > 0){
                  write.csv(result$window_gpp_response, 
                            file.path(group_dir, paste0(matched_model, "_", "GPP_self_response_G", group_i, "_window_", window_label, ".csv")), row.names = FALSE)
                }
                if(nrow(result$window_gpp_irf) > 0){
                  write.csv(result$window_gpp_irf, 
                            file.path(group_dir, paste0(matched_model, "_", "GPP_irf_response_G", group_i, "_window_", window_label, ".csv")), row.names = FALSE)
                }
    ...
    # base_path <- "/data/share/tangwenxi/Out-Table-and-Figure-VPD et al/2026-03-09/GroupBy_100_SlidingWindow_39_yr"
    base_path <- Datedir
    
    # Get all vegetation indicator folders (GPP, LAI, NDVI)
    indicators <- list.dirs(base_path, full.names = FALSE, recursive = FALSE)
    
    # 2. Loop through each indicator
    for (ind in indicators) {
      ind_path <- file.path(base_path, ind)
      
      # Get all Window folders under the current indicator
      windows <- list.dirs(ind_path, full.names = FALSE, recursive = FALSE)
      
      for (win in windows) {
        win_path <- file.path(ind_path, win)
        
        # Get all Group directories and perform natural sorting (Group_1, Group_2, ..., Group_10)
        group_dirs <- list.dirs(win_path, full.names = TRUE, recursive = FALSE)
        group_dirs <- mixedsort(group_dirs)
        
        # Get all CSV file names in the first group folder to determine output file types (e.g., ADF, causal, etc.)
        # Assumes that all group folders share the same file structure
        sample_files <- list.files(group_dirs[1], pattern = "\\.csv$", full.names = FALSE)
        
        # Extract file type suffixes (e.g., extract "ADF" from "GPP_G1_window_1982_2020_ADF.csv")
        # Logic: Extract the substring between the last underscore and .csv
        file_types <- unique(str_extract(sample_files, "[^\\_]+(?=\\.csv$)"))
        
        # 3. Loop through and merge each file type
        for (type in file_types) {
          cat(paste0("Merging: ", ind, " -> ", win, " -> Type: ", type, "...\n"))
          
          # Find file paths matching the current type across all sorted group directories
          all_file_paths <- unlist(lapply(group_dirs, function(d) {
            list.files(d, pattern = paste0("_", type, "\\.csv$"), full.names = TRUE)
          }))
          
          # Read and merge files if any are found
          if (length(all_file_paths) > 0) {
            # Read all CSV files into a list
            dt_list <- lapply(all_file_paths, function(f) {
              dt <- fread(f)
              # Optional: Add a column to track which Group the row came from
              # dt[, source_group := str_extract(f, "Group_\\d+")]
              return(dt)
            })
            ...
            # 4. Save the combined CSV file
            # Destination: Saved inside a designated output folder under the base path
            output_name <- paste0(Model, "_", "Combined_", ind, "_", win, "_", type, ".csv")
            output_dir <- file.path(base_path, paste0("Combined_", Model))
            if (!dir.exists(output_dir)) dir.create(output_dir)
            
            fwrite(combined_dt, file.path(output_dir, output_name))
          }
        }
      }
    }
  ...
  # dirs <- list.dirs('/data/share/tangwenxi/Out-Table-and-Figure-VPD et al/Combined_Results', recursive = F)
  # list.files(dirs[1])
  # data_dir <- '/data/share/tangwenxi/Out-Table-and-Figure-VPD et al/Combined_Results'
  
  # hist_path <- file.path(data_dir, 'Combined_1982_2020', "Combined_GPP_Window_1982_2020_irf.csv")
  # hist_data <- read.csv(hist_path)
  
  # library(dplyr)
  # hist_data <- hist_data %>% dplyr::select(-V1)
  # head(hist_data)
  # table(hist_data$variable)
